...
- Resolve every job by its declared `profileId`; never select the first active profile or require that only one browser be open.
...
- Keep at most one active job per AdsPower Profile and one active job per Meta ad account. Jobs sharing either key must queue even when global concurrency is higher.
...
2. Run a read-only probe for every declared Profile. Do not mutate any job until every job has a resolved Profile, exactly one intended Ads Manager tab, matching account/page/currency, and a classified gate.
...
4. Use the single-job Skill to create and save each draft. Checkpoint jobs as `PREFLIGHT_OK`, `DRAFT_SAVED`, `FAILED`, or `HUMAN_ACTION_REQUIRED`.
5. Report the complete draft review table. If fresh publish authorization is absent or narrower than the batch, stop all unauthorized jobs at `DRAFT_SAVED`.
6. For authorized jobs only, re-probe immediately before publishing, then publish with the configured publish concurrency while retaining per-account serialization.
7. Confirm each result from Meta. Record `PUBLISHED`, `FAILED`, or `UNKNOWN`; do not claim batch success from the click alone.
